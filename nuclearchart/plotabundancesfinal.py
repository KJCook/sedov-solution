...
Znetwork = np.asarray([row[2] for row in network])
Nnetwork = np.asarray([row[3] for row in network])
Anetwork = Znetwork + Nnetwork
...
data = np.genfromtxt('evfe1.dat', dtype = float) 
...
#print np.shape(color_data)
lasttime=color_data[len(color_data)-1]
#print lasttime 
...
scatt = ax.scatter(Anetwork,lasttime)
#ax.text(0,25,'time')
ax.set_xlabel("Mass Number",fontsize=12)
